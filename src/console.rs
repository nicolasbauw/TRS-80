...
pub fn launch(
    cmd_channel: mpsc::Sender<(String, String)>,
) -> Result<(), Box<dyn std::error::Error>> {
    thread::Builder::new().name(String::from("Console")).spawn(
        move || -> Result<(), mpsc::SendError<(String, String)>> {
            loop {
                print!("> ");
                ...
                match command {
                    "tape" => match args.peekable().peek() {
                        Some(tape) => {
                            cmd_channel.send((String::from(command), tape.to_string()))?
                        }
                        None => {
